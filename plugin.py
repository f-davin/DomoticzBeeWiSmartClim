...
class BasePlugin:
    ...
    def onCommand ( self, Unit, Command, Level, Hue ):
        Domoticz.Log (
            "onCommand called for Unit " + str ( Unit ) + ": Parameter '" + str ( Command ) + "', Level: " + str (
                Level
                )
            )
    ...
    def getHumidityStatus ( self, temperature, humidity ):
        ...
        HI = c1 + c2 * temperature + c3 * humidity + c4 * temperature * humidity + c5 * (
                temperature * temperature) + c6 * (humidity * humidity)
        HI = HI + c7 * (temperature * temperature) * humidity + c8 * temperature * (humidity * humidity) + c9 * (
